  ...
  try {
    ...
    if (existingConsole) {
      return res
        .status(400)
        .json(
          `That console is already registered with the ID: ${existingConsole.id}`
        );
    }
...
const deleteConsole = async (req, res, next) => {
  try {
    ...
    if (deletedConsole) {
      return res
        .status(200)
        .json({ mensaje: 'Console deleted', juego: deletedConsole });
    } else {
      return res.status(404).json('Console not found');
    }
  } catch (error) {
    return res.status(400).json(error);
  }
...
const getConsoles = async (req, res, next) => {
  try {
    ...
    if (allConsoles.length === 0) {
      return res
        .status(204)
        .json('There are no consoles in the database');
    }
